...
class KickAI(AIInterface):
    ...
    def get_information(self, frame_data: FrameData, is_control: bool):
        ...
        # --- ADDED BACKEND CHECK BELOW ---
        if frame_data.current_frame_number % 15 == 0:
            try:
                res = requests.get("http://localhost:3000/next-command", timeout=0.05)
                if res.status_code == 200:
                    data = res.json()
                    cmd = data.get("command")
                    if cmd:
                        logger.info(f"[TWITCH] Command received: {cmd}")
